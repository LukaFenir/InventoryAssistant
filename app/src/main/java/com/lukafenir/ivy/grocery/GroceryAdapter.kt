    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val bgColor = if (isSelected) {
            MaterialColors.getColor(holder.itemView, androidx.appcompat.R.attr.colorPrimary).let {
                android.graphics.Color.argb(51, android.graphics.Color.red(it), android.graphics.Color.green(it), android.graphics.Color.blue(it))
            }
        } else {
            Color.TRANSPARENT
        }
        ...
        if (isInSelectionMode) {
            ...
            holder.groceryCheckBox.isClickable = false
            holder.groceryCheckBox.isFocusable = false
            holder.itemView.setOnClickListener { onItemClick(item) }
            holder.itemView.setOnLongClickListener { onLongClick(item); true }
        } else {
            holder.groceryCheckBox.isClickable = true
            holder.groceryCheckBox.isFocusable = true
            holder.groceryCheckBox.setOnCheckedChangeListener(null)
